...
class ElectionResults():
    ...
    def __init__(self, output: ExperimentOutput) -> None:
        ...
        self.choices = {
            task:output.task_configs[task]["dataset_kwargs"]["choices"]
            for task in tasks
        }
        self.columns = {
            task:output.task_configs[task]["dataset_kwargs"]["columns"]
            for task in tasks
        }
...
def get_relative_error(
    ...
    blue_err = (
        norm_prob_df.drop("U.S.")["Democratic"].
        apply(lambda x: x.sub(voting.blue_pct).abs().div(voting.blue_pct))
    )
    ...
    red_err = (
        norm_prob_df.drop("U.S.")["Republican"].
        apply(lambda x: x.sub(voting.red_pct).abs().div(voting.red_pct))
    )
...
def get_counts(diff: pd.DataFrame, agreement: pd.DataFrame) -> pd.DataFrame:
    ...
    objs = {
        "counts (D/R)": counts,
        "true D": blue_tp_counts,
        "true R": red_tp_counts
    }
