...
def nll_styler(df: pd.DataFrame) -> pd.DataFrame.style:
    return df.style.background_gradient(cmap="RdYlGn_r", vmin=0, vmax=10)
...
def prob_styler(df: pd.DataFrame) -> pd.DataFrame.style:
    def highlight_min_max(s: pd.Series) -> list:
        ...
        max_val = s.max()
        return [
            "background-color: #ea9999; color: black"
            if v == min_val
            else "background-color: #a4c2f4; color: black"
            if v == max_val
            else ""
            for v in s
        ]
...
def stats_styler(df: pd.DataFrame) -> pd.DataFrame.style:
    ...
    styled_df = (
        df.style.map(fn, subset=us_subset)
