...
#. # # #
st.subheader("OpenAI Compatible Server [🔗](https://https://docs.vllm.ai/en/latest/serving/openai_compatible_server.html)")
_, input_col, req_col, _  = st.columns([.2, .3, .3, .2])
#. _ # _
_, log_col, _ = st.columns([.2, .52, .2])
with log_col:
    log_widget = st.empty()
    
with input_col:
    st.write("###### vLLM Engine Arguments [🔗](https://docs.vllm.ai/en/latest/serving/openai_compatible_server.html#cli-reference)")
...
with req_col:
    st.markdown("###### Connect to OpenAI Compatible Server [🔗](https://docs.vllm.ai/en/latest/serving/openai_compatible_server.html#api-reference)")
    with st.form("form"):
