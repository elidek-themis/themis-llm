...
def get_groups(samples: list) -> pd.DataFrame:
    df = pd.DataFrame(
        data=[(s["doc"]["type"], s["pct_stereotype"]) for s in samples], columns=["bias_type", "pct_stereotype"]
    )
...
with b_v_it:
    columns = ["pct_stereotype", "likelihood_diff"]
